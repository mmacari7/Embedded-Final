...
unsigned long ConvertToDistance(unsigned long sample){
   // A is slope and B is intercept of calibration graph
	//For 3 cm long potentiometer with 10 kohm resistance:
	   //return ((1150*sample) >> 10) + (429 >> 10); //A was found to be 0.7326, right shift by 10 is division by 1024, 750/1024 is approximately 0.7326
		return sample/62;															 //B was found to be 0.4188, right shift by 10 is division by 1024, 429/1024 is approximately 0.4188
...
void PlayerMove(void){
	Player.GObj.x = (ConvertToDistance(ADC0_In())); 
}
...
unsigned char Check_GameOver(void){
	if((KilledEnemyCount == 8) || (Player.GObj.life == 0))
		return 1;
	return 0;
}
...
void State_GameOver(void){
  ...
  Nokia5110_SetCursor(1, 1);
	if(KilledEnemyCount == 8){
		Nokia5110_OutString(" WOOOOH!");
		Nokia5110_SetCursor(1, 2);
		Nokia5110_OutString("You won,");
	}
	else{
		Nokia5110_OutString("better luck");
		Nokia5110_SetCursor(1, 2);
		Nokia5110_OutString("next time,");
	}
